...
def register_callbacks(app):
    ...
    def update_drill_stores(chart_clicks, tab, category, d1, d2, city_drill, svc_drill):
        ...
        if isinstance(triggered, dict) and triggered.get("type") == "drill-chart":
            ...
            if click_data and "points" in click_data:
                ...
                if not label: return no_update, no_update
                if triggered_idx == "city":
                    if len(city_drill) == 0: return [label], svc_drill
                    elif len(city_drill) == 1: return [city_drill[0], label], svc_drill
                elif triggered_idx == "services":
                    if len(svc_drill) == 0: return city_drill, [label]
                    elif len(svc_drill) == 1: return city_drill, [svc_drill[0], label]
                    elif len(svc_drill) == 2: return city_drill, [svc_drill[0], svc_drill[1], label]
    ...
    def render_drilldown_panel(tab, category, d1, d2, city_drill, svc_drill):
        # Render panel conditionally based on updated drilldown store contexts
        return _render_drilldown(tab, category, d1, d2, city_drill, svc_drill)
...
def _render_drilldown(tab, category, d1, d2, city_drill, svc_drill):
